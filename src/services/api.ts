...
async function fetchWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`${name} attempt ${attempt}/${maxRetries} failed:`, lastError.message);
